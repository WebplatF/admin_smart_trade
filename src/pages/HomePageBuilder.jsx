    ...
    try {
      ...
      if (res.data.status) {
        ...
        await Promise.all([
          ...
          (async () => {
            const wb = [...bannerList].reverse().find(item => item.title === "website");
            if (!wb?.path) return;
            try {
              const r = await getWasabiFile(wb.path);
              map["b_web"] = r?.data?.data?.wasabi_url ?? null;
            } catch {
              map["b_web"] = null;
            }
          })(),
          ...
          // (async () => {
          //   const odds = bannerList.filter(item => item.title === "mobile");
          ...
          // })(),
          (async () => {
            const mb = [...bannerList].reverse().find(item => item.title === "mobile");
            if (!mb?.path) return;
            try {
              const r = await getWasabiFile(mb.path);
              map["b_mobile"] = r?.data?.data?.wasabi_url ?? null;
            } catch {
              map["b_mobile"] = null;
            }
          })(),
...
const evenBanners  = banners.filter(item => item.title === "website"); // web banners
const oddBanners   = banners.filter(item => item.title === "mobile");  // mobile banners
          ...
          {mobileBanner ? (
            <div className="hb-banner-preview">
              <div className="hb-banner-img-box">
                ...
                {/* <span className="hb-device-badge"><Monitor size={10} /> Mobile</span> */}
              </div>
              <div className="hb-banner-meta">
                <span className="hb-banner-name">{mobileBanner.title}</span>
                <button className="hb-change-btn" onClick={() => { setBannerErr(""); setSelectedImg(null); setImages([]); setBannerModal(mobileBanner.title); }}>
                  Change
                </button>
              </div>
            </div>
          ) : (
            <div className="hb-device-empty" onClick={() => { setBannerErr(""); setSelectedImg(null); setImages([]); setBannerModal("mobile"); }}>
        ...
        <div className="hb-device-slot">
          ...
          {webBanner ? (
            <div className="hb-banner-preview">
              <div className="hb-banner-img-box">
                {itemUrls["b_web"]
                  ? <img src={itemUrls["b_web"]} alt={"Website"} />
                  : <div className="hb-img-ph"><ImageIcon size={28} color="#c5d0f5" /></div>}
                {/* <span className="hb-device-badge hb-device-badge--mobile"><Smartphone size={10} /> Website</span> */}
              </div>
              <div className="hb-banner-meta">
                <span className="hb-banner-name">{webBanner.title}</span>
                <button className="hb-change-btn hb-change-btn--mobile" onClick={() => { setBannerErr(""); setSelectedImg(null); setImages([]); setBannerModal(webBanner.title); }}>
                  Change
                </button>
              </div>
            </div>
          ) : (
            <div className="hb-device-empty hb-device-empty--mobile" onClick={() => { setBannerErr(""); setSelectedImg(null); setImages([]); setBannerModal("website"); }}>
